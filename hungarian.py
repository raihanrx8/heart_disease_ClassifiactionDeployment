...
# # Add this function to load custom CSS
# def load_css(file_path: str):
#     with open(file_path) as f:
#         st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)

# # Call the function to load custom CSS
# load_css('custom.css')  # Replace 'custom.css' with the actual file name
...
st.write("**Nama** : Muhammad Raihan Firmansyah")
st.write("**NIM** : A11.2020.13184")
...
with tab1:
    ...
    chol = st.sidebar.number_input(label=":violet[**Serum cholestoral** (in mg/dl)]", min_value=df_final['chol'].min(),
